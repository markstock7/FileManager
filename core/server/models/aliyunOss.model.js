...
function aliyunOss(settings) {
  this.aliyunOssSetting = settings;
  console.log(typeof this.aliyunOssSetting);
}

aliyunOss.prototype.listBuckets = function listBuckets(options) {
  var tasks = [] ,aliyunEndpoint;
  for( endpoint in config.aliyunOssSetting) {
    // 默认取第一个然后直接跳出
    aliyunEndpoint = config.aliyunOssSetting[endpoint];
    var listBuckets = Promise.promisify(aliyunEndpoint.listBuckets);
    aliyunEndpoint.listBuckets = listBuckets;
    break;
  }
  return aliyunEndpoint.listBuckets().then(function(result) {
      var buckets = [];
      _.forEach(result.Buckets, function(bucket) {
        buckets.push({
            Name: bucket.Name,
            Addition: bucket
        });
...
aliyunOss.prototype.listObjects = function listObjects(options) {
  var aliyunEndpoint = this.aliyunOssSetting['http://' + options.Addition.ExtranetEndpoint];
  // for(fn in aliyunEndpoint){
  //   console.log(aliyunEndpoint));
  // }
  // console.log(aliyunEndpoint[listObjects].toString());
  aliyunEndpoint.listObjects = Promise.promisify(aliyunEndpoint.listObjects);
  var dataGetter;
  if (options.key === '/') {
    dataGetter = aliyunEndpoint.listObjects({
  ...
  return dataGetter.then(function(result) {
    console.log(result);
    var objects = {
        files: [],
        folders: []
    ...
    _.each(result.Contents, function(object) {
      ...
      object.address = 'http://' + options.bucket + '.'+options.Addition.ExtranetEndpoint + '/' +object.Key;
      objects.files.push(object);
    });
    _.each(result.CommonPrefixes, function(object){
      object.Name = object.Prefix;
      object.Key = object.Prefix;
      objects.folders.push(object);
    });
    return objects;
  });
}
module.exports = new aliyunOss(config.aliyunOssSetting);
